...
#define PROID			"xs2N8isfap"

#define ACCESS_KEY		"c21oY2ZaMTVMSmpydGdSZDFsSEdsVDNQckNLOGxGWmY="
...
// MAX30102 variables
extern u8 filtered_hr, filtered_spo2;
extern u16 mq3_value;
extern u8 alarm_state;
extern u16 MQ3_THRESHOLD;
...
unsigned char OneNet_FillBuf(char *buf)
{
	...
	memset(text, 0, sizeof(text));
	sprintf(text, "\"mq3\":{\"value\":%d},", mq3_value);
	strcat(buf, text);
	
	memset(text, 0, sizeof(text));
	sprintf(text, "\"xl\":{\"value\":%d},", filtered_hr);
	strcat(buf, text);
	
	memset(text, 0, sizeof(text));
	sprintf(text, "\"xy\":{\"value\":%d},", filtered_spo2);
	strcat(buf, text);
	
	memset(text, 0, sizeof(text));
	sprintf(text, "\"jjyz\":{\"value\":%d},", MQ3_THRESHOLD);
	strcat(buf, text);
	
	memset(text, 0, sizeof(text));
	sprintf(text, "\"bj\":{\"value\":%s}", (alarm_state > 0) ? "true" : "false");
	strcat(buf, text);
...
void OneNet_RevPro(unsigned char *cmd)
{
	...
	cJSON *raw_json, *params_json, *led_json, *jjyz_json;
		...
		case MQTT_PKT_PUBLISH:																//���յ�Publish��Ϣ
			...
			if(result == 0)
			{
				...
				raw_json = cJSON_Parse(req_payload);
						...
						// 处理酒精阈值设置
						jjyz_json = cJSON_GetObjectItem(params_json,"jjyz");
						if(jjyz_json != NULL && jjyz_json->type == cJSON_Number)
						{
							MQ3_THRESHOLD = jjyz_json->valueint;
							UsartPrintf(USART_DEBUG, "Updated MQ3 threshold to: %d\r\n", MQ3_THRESHOLD);
						}
